...
def everything_for_your_cat(cats_data):
    ...
    for cat_data in cats_data:
        cat_name, cat_age, owner_name, owner_surname = cat_data
        owner_key = owner_name + ' ' + owner_surname
        if owner_key in owners:
            owners[owner_key].append((cat_name, cat_age))
        else:
            owners[owner_key] = [(cat_name, cat_age)]
    result = ''
    for owner_key, cats in owners.items():
        owner_name, owner_surname = owner_key.split()
        cat_info = '; '.join([f'{cat_name}, {cat_age}' for cat_name, cat_age in cats])
        result += f'{owner_name} {owner_surname}: {cat_info}\n'
    return result
